export class CurrencyInput {
  ...
  format(event) {
    ...
    const jumpAhead =
      this.target.value.length == 1 && sap.displayBefore
        ? sap.symbol.length
        : 0;
    if (this.onChange) {
      this.onChange(this.getValueAsFloat() || 0);
    }
    if (typeof event != "undefined") {
      input = this.getEventValue(event).replaceAll(sap.symbol, "");
      var sepBef = separationChar != "" ? input.length : 0;
      if (separationChar != "")
        input = input.replaceAll(new RegExp("\\" + separationChar, "g"), "");
      input = input
        .replaceAll(new RegExp("\\" + decimalChar, "g"), ".")
        .replaceAll(/\s/g, "");
      const decimalRegex = new RegExp(
        `^[^\\.]*\\.(\\d{0,${this.getCurrencyDecimalCount()}})?$`
      );
    ...
    input = input.replaceAll(
      new RegExp("\\" + this.getDecimalCharacter(true), "g"),
      "/"
    );
    input = input.replaceAll(
      new RegExp("\\" + this.getSeparationCharacter(true), "g"),
      separationChar
    );
    ...
    if (sap.displayBefore) {
      this.target.value =
        input < 0 ? "-" + sap.symbol + input.substring(1) : sap.symbol + input;
    } else {
  ...
  reinit(options) {
    ...
    if (
      typeof options.currency != "undefined" &&
      !this.currencyCodes.includes(options.currency)
    )
      throw new TypeError(
        options.currency +
          "is an unknown currency code. Refer to ISO 4217 for a list of valid codes or leave empty to default to USD."
      );
    this.currency =
      typeof options.currency == "string" ? options.currency : "USD";
    this.locale = typeof options.locale == "string" ? options.locale : "en-US";
    this.currencyDisplay =
      typeof options.currencyDisplay == "string"
        ? options.currencyDisplay
        : "symbol";
    this.currencyDisplayFallback =
      typeof options.currencyDisplayFallback == "string"
        ? options.currencyDisplayFallback
        : "code";
    ...
    if (this.getDecimalCharacter() == this.getSeparationCharacter()) {
      throw new TypeError(
        "Decimal and separation characters must not be the same."
      );
    }

    if (
      this.getDecimalCharacter() == "/" ||
      this.getSeparationCharacter() == "/"
    ) {
      throw new TypeError(
        "The devider characters must not be a forward slash."
      );
    }

    this.disableCents =
      typeof options.disableCents == "boolean" && options.disableCents == true
        ? true
        : false;
    this.preventInputFromIME =
      typeof options.preventInputFromIME != "undefined" &&
      typeof options.preventInputFromIME == "boolean" &&
      options.preventInputFromIME == false
        ? false
        : true;
    this.min = typeof options.min == "number" ? options.min : false;
    this.max =
      typeof options.max == "number" &&
      (options.max >= this.min || this.min === false)
        ? options.max
        : false;
    this.validCallback =
      typeof options.validCallback == "function"
        ? options.validCallback
        : false;
    this.invalidCallback =
      typeof options.invalidCallback == "function"
        ? options.invalidCallback
        : false;
    this.onChange =
      typeof options.onChange == "function" ? options.onChange : false;
  ...
  getCurrencySymbolAndPosition() {
    if (this.currencyDisplay === "none")
      return { displayBefore: false, symbol: "" };
    ...
    if (
      ...
      (symbol.replaceAll(/\d/g, "").trim() == this.currency &&
        this.currencyDisplayFallback == "name")
    ) {
      return {
        displayBefore: false,
        symbol:
          " " +
          new Intl.DisplayNames(this.locale, { type: "currency" }).of(
            this.currency
          ),
      };
    }
    ...
    if (
      ...
      symbol.replaceAll(/\d/g, "").trim() == this.currency &&
      this.currencyDisplayFallback == "none"
    ) {
      return {
        displayBefore: false,
        symbol: "",
  ...
  getCurrencyDecimalCount() {
    const decimalTest = new Intl.NumberFormat("en", {
      style: "currency",
      currency: this.currency,
      signDisplay: "never",
      currencyDisplay: "code",
  ...
  getEventValue(e) {
    ...
    const isTryingToSelectAll = e.keyCode === 65 && metKeyPressed;
    const isTryingToCopy = e.keyCode === 67 && metKeyPressed;
    const isTryingToCut = e.keyCode === 88 && metKeyPressed;
    const isTryingToPaste = e.keyCode === 86 && metKeyPressed;
    ...
    if (
      e.type == "keydown" &&
      (clearKey.includes(e.key) || e.ctrlKey) &&
    ...
    if (e.type == "keydown" && (e.key == "Backspace" || e.key == "Delete")) {
      ...
      if (e.target.selectionStart != e.target.selectionEnd) {
        ...
        if (e.key == "Delete") {
          return (
            e.target.value.slice(0, e.target.selectionStart) +
            e.target.value.slice(e.target.selectionEnd + 1)
          );
        } else {
          return (
            e.target.value.slice(0, e.target.selectionStart - 1) +
            e.target.value.slice(e.target.selectionEnd)
          );
        }
      }
  ...
  validateInput(e) {
    ...
    let input = this.getEventValue(e)
      .replaceAll(sap.symbol, "")
      .replaceAll(/\s/g, "");
    if (sepChar != "")
      input = input.replaceAll(new RegExp("\\" + sepChar, "g"), "");
    const decimalChar = this.getDecimalCharacter();
    const decimals = this.getCurrencyDecimalCount();
    let regex = "^";
    ...
    if (input == "-") {
      ...
      if (input.slice(-1) == decimalChar) {
        var checkValue = parseFloat(
          input.replace(new RegExp("\\" + decimalChar), ".") +
            Math.pow(10, -decimals).toString().replace("0.", "")
        );
      } else {
        var checkValue = parseFloat(
          input.replace(new RegExp("\\" + decimalChar), ".")
    ...
    if (
      input.match(new RegExp(regex)) === null ||
      (this.min !== false && checkValue < this.min) ||
      (this.max !== false && checkValue > this.max) ||
      checkValue >
        999999999999999 / Math.pow(10, this.disableCents ? 0 : decimals) ||
      checkValue <
        -999999999999999 / Math.pow(10, this.disableCents ? 0 : decimals) ||
      this.getEventValue(e).includes("-" + this.getSeparationCharacter())
    ) {
      if (this.preventInputFromIME && e.type == "compositionend")
        this.target.value = this.target.value.replace(e.data, "");
      if (
        typeof this.invalidCallback == "function" &&
        this.target.value != input
      )
        this.invalidCallback();
      if (typeof e.synthetic == "undefined") e.preventDefault();
    } else {
      if (typeof this.validCallback == "function" && input != this.target.value)
        this.validCallback();
